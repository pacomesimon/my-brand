...
let myFunction = () => {
  describe('/POST comments', () => {
    it('it can not POST comments when no x-auth-token is sent in request\'s header', done => {
      ...
      _chai.default.request(_index.default).post('/api/comments').send(comment).end((err, res) => {
    ...
    it('it should not POST a comment with an invalid auth token', done => {
      ...
      _chai.default.request(_index.default).post('/api/comments').set('x-auth-token', 'invalid random stuff aksjd').send(comment).end((err, res) => {
    ...
    it('it should not POST a comment without "commentBody" field', done => {
      ...
      _chai.default.request(_index.default).post('/api/comments').set('x-auth-token', process.authTokens.jane).send(comment).end((err, res) => {
